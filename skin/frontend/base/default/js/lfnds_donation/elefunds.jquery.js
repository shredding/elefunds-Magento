...
var elefunds = (function (parent, $) {
    ...
    function init(options) {
        ...
        var enabled = false,
            cachedCheckbox = $('#elefunds_bottom input[type="checkbox"]');
        ...
        $("#elefunds_checkbox").on('change', function () {
            if ($("#elefunds_checkbox").prop('checked') && $('#elefunds_bottom input[type="checkbox"]:checked').length === 0) {
                cachedCheckbox.prop('checked', true);
                cachedCheckbox.parent().toggleClass("elefunds_receiver_selected");
            }
